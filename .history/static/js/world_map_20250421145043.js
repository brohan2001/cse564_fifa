...
async function loadWorldMapData() {
    try {
        ...
        // Create country-to-data mapping
        const countryDataMap = {};
        globalData.countries.forEach(country => {
            countryDataMap[country.country] = {
                numPlayers: country.num_players,
                avgOverall: country.avg_overall,
                avgValue: country.avg_value
            };
        });
        
        // Find min and max for color scaling
        const playerCounts = globalData.countries.map(c => c.num_players);
        const maxPlayers = d3.max(playerCounts);
        ...
        worldMap.g.selectAll('path')
            ...
            .on('mouseover', function(event, d) {
                ...
                if (countryData) {
                    ...
                    worldMap.tooltip.html(`
                        ...
                        Avg. Value: €${countryData.avgValue.toFixed(1)}M
                    `)
            ...
            .on('click', function(event, d) {
                ...
                const countryLeagues = globalData.leagues.filter(l => l.country === countryName);
                
                if (countryLeagues.length > 0) {
                    // If there's only one league in the country, select it directly
                    if (countryLeagues.length === 1) {
                        const leagueName = countryLeagues[0].league_name;
                        ...
                        // If multiple leagues, show a popup or update the filters to allow selection
                        showLeagueSelectionForCountry(countryName, countryLeagues);
                    }
                }
            });
