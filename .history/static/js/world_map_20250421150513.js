...
function getCountryForLeague(leagueName) {
    ...
    if (leagueName.includes('England')) return 'United Kingdom';
    if (leagueName.includes('Spain')) return 'Spain';
    if (leagueName.includes('Italy')) return 'Italy';
    if (leagueName.includes('Germany')) return 'Germany';
    if (leagueName.includes('France')) return 'France';
    if (leagueName.includes('Portugal')) return 'Portugal';
    if (leagueName.includes('Netherlands')) return 'Netherlands';
    if (leagueName.includes('Brazil')) return 'Brazil';
    if (leagueName.includes('Argentina')) return 'Argentina';
    if (leagueName.includes('USA') || leagueName.includes('MLS')) return 'United States';
...
async function loadWorldMapData() {
    try {
        ...
        // FIFA league to country mapping
        ...
        globalData.players.forEach(player => {
            if (player.league_name) {
                ...
                leaguePlayerCounts[player.league_name].totalOverall += player.player_overall;
                leaguePlayerCounts[player.league_name].totalValue += player.value_millions_eur;
            }
        });
        ...
        Object.entries(leaguePlayerCounts).forEach(([league, data]) => {
            const country = leagueCountryMap[league] || getCountryForLeague(league);
            ...
            .on('click', function(event, d) {
                ...
                if (countryData && countryData.leagues.length > 0) {
                    ...
                    if (countryData.leagues.length === 1) {
                        ...
                        // Update UI
                        document.getElementById('league-selector').value = leagueName;
                        updateClubSelector();
                        ...
                        showLeagueSelectionForCountry(countryName, countryData.leagues);
                    }
                }
            });
...
function addCountryLabels(countryDataMap) {
    ...
    significantCountries.forEach(countryData => {
        ...
        const feature = worldMap.worldData.features.find(f => f.properties.name === countryData.country);
...
function showLeagueSelectionForCountry(countryName, leagues) {
    ...
    leagues.forEach((league, i) => {
        ...
        leagueGroup.on('click', function() {
            ...
            // Update UI
            document.getElementById('league-selector').value = league.name;
            updateClubSelector();
            updateSelectionDetails();
